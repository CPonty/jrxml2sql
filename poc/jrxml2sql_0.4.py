...
class SqlParameterFormatter(object):
	...
	def header(self, pCollection_params, 
		...
		if type(pCollection_params)!=JrxmlParameterCollection or\
			type(pCollection_expressions)!=JrxmlParameterCollection:
			raise TypeError("'pCollection' arguments should be a JrxmlParameterCollection")
		if pCollection_params.count()==0: 
			return ""
		header1=self.func_format_header(pCollection_params, 
			'--'+self.editorName+" Parameters--")
		if pCollection_expressions.count()==0: 
			return header1
		header2=self.func_format_header(pCollection_expressions, 
			'--'+self.editorName+" $X{} Replacements--")
...
def workbench_header(pCollection,label):
	def wbvardef(n,v,t): 
...
def sqlmanagerstudio_header(pCollection, label):
	header="DECLARE %s;\n\n"%(
...
class JrxmlSqlParser(object):
	...
	def formatSql(self):
		...
		header=self.sqlParameterFormatter.header(
			self.sqlParams,self.sqlXinParams)
		return header+'--Query--\n'+formatStr
	...
	def findSqlParams(self):
		...
		#find $X{}
		i=0
		match=re.search(r'\$X\{[^\}]{6,256}\}', self.sqlStr[i:])
		while match!=None:
			s=self.sqlStr[i+match.start():i+match.end()]
